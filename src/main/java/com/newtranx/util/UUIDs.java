...
public final class UUIDs {
	...
	/*
	 * The min and max possible lsb for a UUID. Note that his is not 0 and all
	 * 1's because Cassandra TimeUUIDType compares the lsb parts as a signed
	 * byte array comparison. So the min value is 8 times -128 and the max is 8
	 * times +127.
	 *
	 * Note that we ignore the uuid variant (namely, MIN_CLOCK_SEQ_AND_NODE have
	 * variant 2 as it should, but MAX_CLOCK_SEQ_AND_NODE have variant 0)
	 * because I don't trust all uuid implementation to have correctly set those
	 * (pycassa don't always for instance).
	 */
	...
	private static long makeEpoch() {
		// UUID v1 timestamp must be in 100-nanoseconds interval since
		// 00:00:00.000 15 Oct 1582.
	...
	private static long makeNode() {
		...
		try {
			...
			// bit (least significant bit of the first byte of the node ID) must
			// be 1.
	 ...
	 * <p/>
	 * Note that there is no way to provide your own timestamp. This is
	 * deliberate, as we feel that this does not conform to the UUID
	 * specification, and therefore don't want to encourage it through the API.
	 ...
	 * </pre>
	 * 
	 * If you simply need to perform a range query on a {@code timeuuid} column,
	 * use the "fake" UUID generated by {@link #startOf(long)} and
	 * {@link #endOf(long)}.
	 *
	 ...
	 * user-provided timestamps are not supported (see {@link #timeBased()} for
	 * more explanations).
	 * <p/>
	 * Also, the timestamp to provide as parameter must be a unix timestamp (as
	 * returned by {@link System#currentTimeMillis} or
	 * {@link java.util.Date#getTime}), not a UUID 100-nanoseconds interval
	 ...
	 *
	 * @param timestamp
	 *            the unix timestamp for which the created UUID must be a lower
	 *            bound.
	 ...
	 * version 1 UUID generated at the provided timestamp.
	 * <p/>
	 * See {@link #startOf(long)} for explanations about the intended usage of
	 * such UUID.
	 *
	 * @param timestamp
	 *            the unix timestamp for which the created UUID must be an upper
	 *            bound.
	 ...
	 * comparable to the timestamp returned by {@link System#currentTimeMillis},
	 * {@link java.util.Date#getTime}, etc.
	 *
	 * @param uuid
	 *            the UUID to return the timestamp of.
	 ...
	 * previous generation, we increment the number of nanoseconds. However,
	 * since the precision is 100-nanoseconds intervals, we can only generate
	 * 10K UUID within a millisecond safely. If we detect we have already
	 * generated that much UUID within a millisecond (which, while admittedly
	 * unlikely in a real application, is very achievable on even modest
	 * machines), then we stall the generator (busy spin) until the next
	...
	private static Set<String> getAllLocalAddresses() {
		...
		try {
			...
			// Ignore, if we've really got nothing so far, we'll throw an
			// exception
		}
